...
data = fits.open(dic_init['data_cross'])[1].data 

xi = m.valueCross( data.RP, data.RT, data.Z, dic_init)
if met: 
    xi += met.valueCross(dic_init)
...
if args.plot:
    P.figure()
    P.pcolormesh( sp.reshape(data.RP, (100, 50)), \
                  sp.reshape(data.RT, (100, 50)), \
                  sp.reshape(xid*(data.RP**2+data.RT**2), (100, 50)), \
                  vmin=-10, vmax=10. )
    P.colorbar()

    #for mus in [[0., 0.5], [0.5, 0.8], [0.8, 0.95], [0.95, 1.]]:
    for mus in [[-1., -0.5], [-0.5, 0.], [0., 0.5], [0.5, 1.]]:
        w = wedge(rpmin=-200.0, rpmax=200.0, nrp=100, \
        ...
        P.rcParams.update({'font.size':22}) #sets all the font to size 22
