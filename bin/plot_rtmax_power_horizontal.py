...
def get_Cls(rtmax, kinput, nside):
    '''Open estimated kappa files, mask them and calculate their C_ells'''
    ##- Open kappa true-correlation files
    kest = fits.open('maps/midpoint/true_corr/kappa_xnoisy_rt{}.fits.gz'.format(rtmax))[1].data.kappa
    wkest = fits.open('maps/midpoint/true_corr/kappa_xnoisy_rt{}.fits.gz'.format(rtmax))[1].data.wkappa
...
#nside=int(hp.npix2nside(kinput.size))
nside=256
...
for i in a:
    inp, est = get_Cls(i, kinput, nside)
    Cl_in.append(inp)
    Cl_est.append(est)

##- Setup figures
P.rcParams.update({'font.size':12})
P.ion()
ncolors=10
colors = P.cm.jet(np.linspace(0,1,ncolors))

##- Setup 3 subplots with right-hand axis labels
gs = gridspec.GridSpec(1, 3, hspace = 0.2, wspace=0.3)
fig = P.figure(figsize=(14,5))
ax1 = P.subplot(gs[0, 0]) # row 0, col 0
ax2 = P.subplot(gs[0, 1]) # row 0, col 1
ax3 = P.subplot(gs[0, 2]) # row 0, col 2
P.setp(ax3, xticks=[0, 200, 400, 600, 800], xlim=([0, 800]))

##- Plot figure
for i in np.arange(10):
    ax1.plot(Cl_in[i], color=colors[i], lw=2, linestyle="-", label="rtmax={}".format(a[i]))
    ax2.plot(Cl_est[i], color=colors[i], lw=2, linestyle="-", label="rtmax={}".format(a[i]))
    ax3.plot(Cl_est[i]/Cl_in[i], color=colors[i], lw=2, linestyle="-", label="rtmax={}".format(a[i]))

ax1.set_ylabel(r'Input (masked) $\ell C_{\ell}^{\kappa \kappa}$', fontsize=14)
ax2.set_ylabel(r'Estimated $\ell C_{\ell}^{\kappa \kappa}$', fontsize=14)
ax3.set_ylabel(r'Estimated / Input $\ell C_{\ell}^{\kappa \kappa}$', fontsize=14)
ax1.set_ylim([0,2e-8])
ax2.set_ylim([0,2e-8])
ax3.set_ylim([0,3.5])
...
ax3.set_xlabel(r'$\ell$', fontsize=14)
...
ax1.legend(by_label.values(), by_label.keys(), numpoints = 1, loc = 'upper right', fontsize=14)

P.savefig('plots/rtmax_horizontal.pdf', bbox_inches='tight')
