## SY 13/3/19
## Plots power spectra for optimal map-making method with different damping.
...
P.plot(ell0, Cl0*ell0/731, color='k', lw=2, linestyle="-", label='Input/731')

k00 = hp.ud_grade(k0, nside)
fin = ['kappa_opt_srad/kappa_cut_rt100_rp100_nside{}_srad01.npz'.format(nside),'kappa_opt_srad/kappa_cut_rt100_rp100_nside{}_srad01_nogaussian.npz'.format(nside)]
fintype=['Gaussian Damping','No Gaussian']
for i,j in enumerate(fin):
    ell, Cl = load_kappa_maps(j, k00)
    P.plot(ell, Cl*ell, color=colors[i], lw=2, linestyle="-", label=fintype[i])
...
P.plot(ell_mid, cl_mid*ell_mid/250, color=colors[2], lw=2, linestyle="-", label='Midpoint/250')


#P.title('Estimated DESI kappa, nside=128, rtmax=100')
...
P.annotate(r'DESI noiseless, nside={}, sradius=0.1, rtmax=rpmax=100'.format(nside), xy=(0.05, 0.95), xycoords='axes fraction')
