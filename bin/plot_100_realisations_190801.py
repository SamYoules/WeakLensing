...
def rebin(cls,ellmin=50, ellmax=750, nell=14):
...
def get_vals(cl_crosses_in, cl_inputs):
    ...
    for i,j in enumerate(cl_crosses_in):
        ell, crosses, cross_wei = rebin(cl_crosses_in[i], ellmin=50, ellmax=750, nell=7)
...
#ax = P.figure(figsize=(8.2,6))

#P.plot(input_mean[0]*x, color=colors[1], linewidth=2.0, linestyle="-",label='Masked Input')
P.plot(input_mean[0]*model[0]*x[0], color="k", lw=2, linestyle="-",label='Masked Input x large-angle damping')
#P.plot(input_mean[1]*model[1]*x[1], color=colors[3], lw=2, linestyle="-",label='Masked Input x large-angle damping (Cross)')
P.errorbar(ell, cross_mean[3]*ell, xerr=None, yerr=cross_stdev[3]*ell, color=colors[2], marker = 'o', fmt='.', capsize=5, elinewidth=2, markeredgewidth=2, markeredgecolor=colors[2], label='Noiseless Auto x Input')
P.errorbar(ell + 6, cross_mean[4]*ell, xerr=None, yerr=cross_stdev[4]*ell, color=colors[3], marker = 'o', fmt='.', capsize=5, elinewidth=2, markeredgewidth=2, markeredgecolor=colors[3], label='Noiseless QSO-Cross x Input')
...
fig, ax = P.subplots(figsize=(7,12))
...
ax1.errorbar(ell, cross_mean[0]*ell, yerr=cross_stdev[0]*ell, 
...
ax3.errorbar(ell, cross_mean[6]*ell, xerr=None, yerr=cross_stdev[6]*ell, color=colors[0], marker = 'o', markeredgecolor=colors[0], fmt='.', capsize=5, elinewidth=2, markeredgewidth=2, label=r'$\langle \kappa_{\alpha \alpha} \kappa_{\rm{input}} \rangle$ High Density')

ax3.errorbar(ell + 4, cross_mean[7]*ell, xerr=None, yerr=cross_stdev[7]*ell, color=colors[1], marker = 'o', markeredgecolor=colors[1], fmt='.', capsize=5, elinewidth=2, markeredgewidth=2, label=r'$\langle \kappa_{q \alpha} \kappa_{\rm{input}} \rangle$ High Density')
...
P.savefig('plots/100_realisations_temp_cov.pdf', bbox_inches='tight')
