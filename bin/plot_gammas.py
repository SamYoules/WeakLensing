...
def plot_mollviews(ga1, ga2, g1true, g2true, kt, kg):

    hp.mollview(ga1, min=-0.5, max=0.5, title = 'gamma1 (blob) estimator')
    P.savefig('plots/gamma1_estim_blob.png')
    P.close()

    hp.mollview(g1true, min=-0.5, max=0.5, title = 'gamma1 (blob) input')
    ...
    hp.mollview(g2true, min=-0.5, max=0.5, title = 'gamma2 (blob) input')
...
def plot_gamma(ga1, ga2, g1true, g2true, mask_size):
    ...
    Cls3=hp.sphtfunc.anafast(g1true, lmax=3*NSIDE-1)
    Cls4=hp.sphtfunc.anafast(g2true, lmax=3*NSIDE-1)
    Cls12=hp.sphtfunc.anafast(ga1, ga2, lmax=3*NSIDE-1)
    Cls13=hp.sphtfunc.anafast(ga1, g1true, lmax=3*NSIDE-1)
    Cls24=hp.sphtfunc.anafast(ga2, g2true, lmax=3*NSIDE-1)

    # gamma1 plot (Reduce cell by with mask to match eBOSS footprint
    #              and by 2 because gamma?)
    P.plot(ell, (cell/2)*mask_size, label='Theory')
    ...
    P.savefig('plots/gamma1_corr_c.png')
    P.close()
    ...
    P.savefig('plots/gamma2_corr_c.png')
    P.close()


def plot_kappa(ga1, ga2, k_k, k_t, mask_size):
    ...
    # The factor below accounts for eBOSS footprint ~ 0.184 of full sky
...
# Make plots
if (outtype == 'g'):
    plot_gamma(g1, g2, g1t, g2t, mask_size)
elif (outtype == 'k'):
    plot_kappa(g1, g2, kap, ktrue, mask_size)
else:
