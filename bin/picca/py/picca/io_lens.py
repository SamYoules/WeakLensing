...
from picca.data_lens import forest # SY 7/2/19
from picca.data_lens import delta # SY 7/2/19
from picca.data_lens import qso # SY 7/2/19
...
def read_drq(drq,zmin,zmax,keep_bal,bi_max=None):
    ...
    fid   = vac[1]["FIBERID"][:]
    z     = vac[1]["Z"][:]                       #SY 8/2/19
    ra0   = vac[1]["RA0"][:].astype('float64')   #SY 8/2/19
    dec0   = vac[1]["DEC0"][:].astype('float64') #SY 8/2/19
    ...
    fid   = fid[w]
    ra0   = ra0[w]*sp.pi/180.
    dec0  = dec0[w]*sp.pi/180.
    vac.close()

    return ra,dec,zqso,thid,plate,mjd,fid,dec0,ra0
...
def read_objects(drq,nside,zmin,zmax,alpha,zref,cosmo,keep_bal=True):
    objs = {}
    ra,dec,zqso,thid,plate,mjd,fid,dec_delens,ra_delens = read_drq(drq,zmin,zmax,keep_bal=True) #SY 8/2/19
    phi = ra
    th = sp.pi/2.-dec
    pix = healpy.ang2pix(nside,th,phi)
    if pix.size==0:
    ...
    for i,ipix in enumerate(upix):
        ...
        w=pix==ipix
        objs[ipix] = [qso(t,r,d,z,p,m,f,dec_delens=d0,ra_delens=r0) for t,r,d,z,p,m,f,d0,r0 in zip(thid[w],ra[w],dec[w],zqso[w],plate[w],mjd[w],fid[w],dec_delens[w],ra_delens[w])] #SY 9/2/19
        for q in objs[ipix]:
