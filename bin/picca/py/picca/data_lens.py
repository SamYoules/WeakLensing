...
class qso:
    def __init__(self,thid,ra,dec,zqso,plate,mjd,fiberid,dec_delens=None,ra_delens=None):
        self.ra = ra
        self.dec = dec
        self.ra_delens = ra_delens
        self.dec_delens = dec_delens
        ...
        if ra_delens is not None:
            self.xcart_delens = sp.cos(ra_delens)*sp.cos(dec_delens)
            self.ycart_delens = sp.sin(ra_delens)*sp.cos(dec_delens)
            self.zcart_delens = sp.sin(dec_delens)
    ...
    def delensed_angle(self, data):
        try:
            x = sp.array([d.xcart_delens for d in data])
            y = sp.array([d.ycart_delens for d in data])
            z = sp.array([d.zcart_delens for d in data])

            cos = x*self.xcart_delens+y*self.ycart_delens+z*self.zcart_delens
            w = cos>=1.
            cos[w]=1.
        except:
            x = data.xcart_delens
...
class forest(qso):
    ...
    def __init__(self,ll,fl,iv,thid,ra,dec,zqso,plate,mjd,fid,order,ra_delens=None,dec_delens=None,  diff=None,reso=None, mmef = None):
        qso.__init__(self,thid,ra,dec,zqso,plate,mjd,fid,ra_delens=ra_delens,dec_delens=dec_delens) #SY 7/2/19
...
class delta(qso):
    ...
    def __init__(self,thid,ra,dec,zqso,plate,mjd,fid,ll,we,co,de,order,iv,diff,m_SNR,m_reso,m_z,dll,ra_delens=None, dec_delens=None):

        qso.__init__(self,thid,ra,dec,zqso,plate,mjd,fid, ra_delens=ra_delens, dec_delens=dec_delens)
        self.ll = ll
        self.we = we
        self.co = co
        self.de = de
    ...
    def from_fitsio(cls,h,Pk1D_type=False):
        ...
        return cls(thid,ra,dec,zqso,plate,mjd,fid,ll,we,co,de,order,
                   ...
                   ra_delens=ra_delens, dec_delens=dec_delens)
