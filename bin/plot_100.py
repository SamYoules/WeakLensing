...
kxi = fits.open('maps/midpoint/true_corr/kappa_xnoisy_rt70.fits.gz')[1].data.kappa
wkxi = fits.open('maps/midpoint/true_corr/kappa_xnoisy_rt70.fits.gz')[1].data.wkappa
#kxi = fits.open('maps/true_corr/kappa_{0}_lensed_true_rt70_rp100.fits.gz'.format(maptype))[1].data.kappa
#wkxi = fits.open('maps/true_corr/kappa_{0}_lensed_true_rt70_rp100.fits.gz'.format(maptype))[1].data.wkappa
...
Clx = ['maps/midpoint/x{}/Cls/Cl_crosses.txt'.format(maptype), '../desi_old/Cls/Cl_crosses_x{}.txt'.format(maptype), '../desi_old/Cls/Cl_crosses_{}.txt'.format(maptype)]
Cla = ['maps/midpoint/x{}/Cls/Cl_autos.txt'.format(maptype), '../desi_old/Cls/Cl_autos_x{}.txt'.format(maptype), '../desi_old/Cls/Cl_autos_{}.txt'.format(maptype)]
Cli = ['maps/input/Cl_inputs.txt', '../desi_old/Cls/Cl_inputs_x{}.txt'.format(maptype), '../desi_old/Cls/Cl_inputs_{}.txt'.format(maptype)]
...
chi2new=r'$\chi^2_{new} = $' + chi21a + r', $\chi^2_{old} = $' + chi21b + r', $\chi^2_{auto} = $' + chi21c
...
P.errorbar(ell, cross_mean[0]*ell, xerr=None, yerr=cross_stdev[0]*ell, color=colors[0], marker = 'o', fmt='.', capsize=5, elinewidth=2, markeredgewidth=2, markeredgecolor=colors[0], label='New {0} Cross x Input'.format(maptype))
P.errorbar(ell + 4, cross_mean[1]*ell, xerr=None, yerr=cross_stdev[1]*ell, color=colors[1], marker = 'o', fmt='.', capsize=5, elinewidth=2, markeredgewidth=2, markeredgecolor=colors[1], label='Old {0} Cross x Input'.format(maptype))
P.errorbar(ell + 8, cross_mean[2]*ell, xerr=None, yerr=cross_stdev[2]*ell, color=colors[2], marker = 'o', fmt='.', capsize=5, elinewidth=2, markeredgewidth=2, markeredgecolor=colors[2], label='Old {0} Auto x Input'.format(maptype))
